...
class _DataTransparencyScreenState
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // Workers view their own report using the shared HMAC key as admin key
      final report = await repo.getWorkerDataReport(
        ApiConfig.hmacSecretKey,
        worker.id,
      );
  ...
  Widget _buildReport(BuildContext context, ThemeData theme, ColorScheme cs,
    ...
    final weatherSnaps = (data['weather_snapshots'] as List<dynamic>?) ?? [];
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.privacy_tip, color: cs.onPrimaryContainer),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(
                    'This is all the data Carbon holds about you, in compliance with the DPDP Act 2023.',
          ...
          _Section(
            ...
            child: Column(children: [
              _Row('Name', worker['name'] ?? '-'),
              _Row('Phone', worker['phone'] ?? '-'),
              _Row('Zone', worker['zone'] ?? '-'),
              _Row('Vehicle', (worker['vehicle_type'] as String? ?? '-').toUpperCase()),
              _Row('Wallet', '₹${(worker['wallet_balance'] as num?)?.toStringAsFixed(2) ?? '0.00'}'),
              _Row('Weekly Rides', '${worker['weekly_rides_completed'] ?? 0}'),
            ]),
          ),
          const SizedBox(height: 16),
          ...
          _Section(
            ...
            icon: Icons.shield,
            child: Column(children: [
              _Row('Active', (policy['is_active'] as bool? ?? false) ? 'Yes' : 'No'),
              if (policy['premium_rate_percent'] != null)
                _Row('Premium Rate', '${policy['premium_rate_percent']}%'),
              if (policy['valid_until'] != null)
                _Row('Valid Until', (policy['valid_until'] as String).substring(0, 10)),
            ]),
          ),
          const SizedBox(height: 16),
          ...
          // Sensor data policy
          _Section(
            title: 'What Sensor Data We Use',
            icon: Icons.sensors,
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _BulletList(
                  label: 'We collect (scalars only)',
                ...
                const SizedBox(height: 8),
                _Row('Retention', sensorPolicy['retention'] ?? '-'),
              ],
            ),
          ),
          const SizedBox(height: 16),
          ...
          // Transactions
          _Section(
            title: 'Your Transactions (${transactions.length})',
          ...
          // Weather snapshots
          _Section(
            title: 'Your Weather Snapshots (${weatherSnaps.length})',
            icon: Icons.cloud,
            child: weatherSnaps.isEmpty
                ? Text('No weather snapshots yet',
                    style: theme.textTheme.bodySmall)
                : Column(
...
class _BulletList extends StatelessWidget {
  ...
  final Color? color;
  const _BulletList({required this.label, required this.items, this.color});
